...
/mob/proc/lord_color_choice()
	...
	var/prim
	var/sec
	var/choice = input(src, "Choose a Primary Color", "BLACKMOOR") as anything in colorlist
	if(choice)
		prim = colorlist[choice]
		colorlist -= choice
	choice = input(src, "Choose a Secondary Color", "BLACKMOOR") as anything in colorlist
	if(choice)
