...
//			continue
	...
	for(var/_AM in listening)
		...
		if(Zs_too && AM.z != src.z && !Zs_all)
			if(!Zs_yell && !HAS_TRAIT(H, TRAIT_KEENEARS))
			...
			if(src != AM && !Zs_yell && !HAS_TRAIT(H, TRAIT_KEENEARS))	//We always hear ourselves. Zs_yell will allow a "!" shout to bypass walls one z level up or below.
