...
/atom/movable/proc/do_item_attack_animation(atom/A, visual_effect_icon, obj/item/used_item, animation_type = ATTACK_ANIMATION_SWIPE)
	...
	if(dist <= 1)
		...
		//Oldschool indicators, used for 2+ reach weapons. The floating sprite doesn't look good in those cases.
		var/turf/first_step = get_step(src, get_dir(src, A))
		if(dist > 1)	//2+ tiles, we flash the indicators
			var/obj/effect/temp_visual/dir_setting/attack_effect/firstatk = new(first_step, get_dir(first_step, A))
